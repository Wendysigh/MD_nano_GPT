...
parser.add_argument('--ckpt_task', type=str, default='Label0.0_window50_interval1_lr0.0005_emb_dim128_l100_block1_scheduled')
...
# duplicate the value in gen_input
...
vocab_size=len(X)
if task == 'lstm':
    model = LSTM (vocab_size, embedding_dim, gen_files , rnn_units, state)
elif task =='trans_gpt':
    model = trans_gpt(vocab_size, seq_lenth, embedding_dim, gen_files, seq_lenth, num_layers=block_num ,inference=True)
...
for i in trange(gen_length):
    ...
    predictions = model(gen_input, None, None, None, None, include_pose=include_pos, training=False)
    if 'trans' in task:
